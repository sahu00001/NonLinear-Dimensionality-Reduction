...
# Define the autoencoder model with increasing neurons in the decoder
def create_autoencoder(input_dim, encoding_dim, encoder_neurons, decoder_neurons):
    ...
    for neuron in encoder_neurons:  # Define encoder structure
        x = Dense(neuron, activation="tanh")(x)

    # Latent space layer
    latent_space = Dense(encoding_dim, activation="tanh")(x)
...
encoding_dim = 118  # Manually setting the encoding dimension
encoder_neurons = [433,1529, 2313]  # Encoder: decreasing number of neurons
decoder_neurons = [2422,1412,1055, 1147]  # Decoder: increasing number of neurons
epochs = 5000  # Number of epochs
batch_size = 67  # Batch size
learning_rate = 0.000010316928101440582  # Custom learning rate
...
autoencoder.save("tanhtrial1updatedtrial12.h5")
...
# Save metrics to CSV
output_csv_file = 'tanhtrial1updatedtrial12.csv'
...
# Save final evaluation metrics to a CSV
final_metrics_df = pd.DataFrame({
    "Metric": ["Final Validation RMSE", "Final Validation MSE", "Final Validation R-squared"],
    "Value": [final_val_rmse, final_val_mse, final_val_r2]
})
