...
def create_autoencoder(input_dim, encoding_dim, encoder_neurons, decoder_neurons):
    ...
    for neuron in encoder_neurons:  # Define encoder structure
        x = Dense(neuron, activation=custom_sigmoid)(x)

    # Latent space layer
    latent_space = Dense(encoding_dim, activation=custom_sigmoid)(x)
...
encoding_dim = 101  # Manually setting the encoding dimension
encoder_neurons = [1819,454]  # Encoder: decreasing number of neurons
decoder_neurons = [1389]  # Decoder: increasing number of neurons
epochs = 5000  # Number of epochs
batch_size = 57  # Batch size
learning_rate = 0.00001915694479719026  # Custom learning rate
...
autoencoder.save("sigmoidtria3updatedtrial79.h5")
...
# Save metrics to CSV
output_csv_file = 'sigmoidtria3updatedtrial79.csv'
